...
// JPL DExxx Uranus's position test main program
...
{
    if (argc < 2) {
        printf ("Usage: uranusT ephemeris_file_name\n");
        return 1;
    } else {
        printf ("\nJPL Uranus position test.  Compare with\n");
	printf ("1996 Astronomical Almanac pages E13 and E34-37\n\n");
    }
    ...
    static char gdate[4][20] = { "Feb 7, 1996", "Apr 27, 1996",
				 "Aug 25, 1996", "Nov 13, 1996" };
    int ix;
    for (ix = 0; ix < 4; ix++) {
	double day = dx[ix];
	// start with heliocentric positions to be compared with
	// Astronomical Almanac page E13
        if (get_planet_posn_vel(tptr, Uranus, Heliocentric, jd, day)) {
	    ...
	    // Astronomical Almanac pages E34-37
	    //  first get the light travel time
	    if (!get_planet_posn_vel(tptr, Uranus, Geocentric, jd, day))
		return 1;
	    d = sqrt(tptr[0] * tptr[0] + tptr[1] * tptr[1] + tptr[2] * tptr[2]);
//	    printf (
...
//		   tptr[3] / get_au(), tptr[4] / get_au(), tptr[5] / get_au());
	    ...
	    if (!get_planet_posn_vel(tptr, Uranus, Geocentric, jd, day_light))
		return 1;
	    ...
	    printf ("  Appar  RA  %s      Dec %s, Dist. %10.8f\n\n",
            ...
            printf ("Uranus position calculation failed\n");
        }
    }
    return 0;
}
