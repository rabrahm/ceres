...
// JPL DExxx Mars's position test main program
...
{
    if (argc < 2) {
        printf ("Usage: marsT ephemeris_file_name\n");
        return 1;
    } else {
        printf ("\nJPL Mars position test.  Compare with\n");
	printf ("1996 Astronomical Almanac pages E12 and E22-25\n\n");
    }
	...
	// Astronomical Almanac pages E12
        if (get_planet_posn_vel(tptr, Mars, Heliocentric, jd, day)) {
	    ...
	    // Astronomical Almanac pages E22-25
	    //  first get the light travel time
	    if (!get_planet_posn_vel(tptr, Mars, Geocentric, jd, day))
		return 1;
	    d = sqrt(tptr[0] * tptr[0] + tptr[1] * tptr[1] + tptr[2] * tptr[2]);
	    ...
	    if (!get_planet_posn_vel(tptr, Mars, Geocentric, jd, day_light))
		return 1;
...
//		   tptr[3] / get_au(), tptr[4] / get_au(), tptr[5] / get_au());
            ...
            printf ("Mars position calculation failed\n");
        }
    }
    return 0;
}
