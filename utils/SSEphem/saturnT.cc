...
// JPL DExxx Saturn's position test main program
...
{
    if (argc < 2) {
        printf ("Usage: saturnT ephemeris_file_name\n");
        return 1;
    } else {
        printf ("\nJPL Saturn position test.  Compare with\n");
	printf ("1996 Astronomical Almanac pages E13 and E30-33\n\n");
    }
	...
	// Astronomical Almanac pages E13
        if (get_planet_posn_vel(tptr, Saturn, Heliocentric, jd, day)) {
	    ...
	    // Astronomical Almanac pages E30-33
	    //  first get the light travel time
	    if (!get_planet_posn_vel(tptr, Saturn, Geocentric, jd, day))
		return 1;
	    d = sqrt(tptr[0] * tptr[0] + tptr[1] * tptr[1] + tptr[2] * tptr[2]);
	    ...
	    if (!get_planet_posn_vel(tptr, Saturn, Geocentric, jd, day_light))
		return 1;
...
//		   tptr[3] / get_au(), tptr[4] / get_au(), tptr[5] / get_au());
            ...
            printf ("Saturn position calculation failed\n");
        }
    }
    return 0;
}
