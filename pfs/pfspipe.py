...
if (pre_process == 1):
	...
	MasterBias, RO_bias, GA_bias = np.zeros((2032,2032)),0,1
...
if ( os.access(NorFlat_fits,os.F_OK) == False ) or (force_flat):
	...
	for order in range(nord):
		...
		for xi in range(Flat.shape[1]):
			yi = np.around(y[xi] - 8).astype('int')
			#print xi,yi
			while yi < Flat.shape[0] and yi < np.around(y[xi] + 9).astype('int') and medianFlat[yi,xi]!=0.:
				pointsy.append(yi)
				pointsx.append(xi)
...
for fsim in thars:
	...
	#hdu.header = hd
	hdu.header.update('SLITNAME', hd['SLITNAME'])
...
for nlisti in range(len(new_list)):
    ...
    if ( os.access(dirout+fout ,os.F_OK) == False ) or (force_spectral_file_build):
        ...
        hdu.header.update('HIERARCH SHUTTER START DATE', h[0].header['UT-DATE'] )
        hdu.header.update('HIERARCH SHUTTER START UT',  h[0].header['UT-TIME'])
        ...
        hdu.header.update('HIERARCH RA',h[0].header['RA-D'])
        hdu.header.update('HIERARCH DEC',h[0].header['DEC-D'])
        ...
        hdu.header.update('HIERARCH OBS LATITUDE',h[0].header['SITELAT'])
        hdu.header.update('HIERARCH OBS LONGITUDE',h[0].header['SITELONG'])
        hdu.header.update('HIERARCH OBS ALTITUDE',h[0].header['SITEALT'])
	...
	#JustExtract = False
	    ...
	    SNR_5130 = np.median(spec[8,26,1000:1101] )
	    ...
	    hdu.header.update('INST', 'PFS')
	    hdu.header.update('RESOL', ref_RES)
